...
class _BusScreenState extends State<BusScreen> {
  ...
  Future<void> _loadNearbyStops() async {
    ...
    try {
      ...
      final overview = await ApiService.instance.getBusOverview(
        location,
      );
  ...
  Future<void> _handleVoiceCommand(
    ...
    if (busNumberMatch != null) {
      await _searchByBusNumber(
        busNumberMatch.group(1)!,
      );
      return;
    }

    if (command.contains('새로고침') ||
        command.contains('갱신')) {
      await _refreshBusData();
      return;
    }
  ...
  Future<void> _searchByBusNumber(
    ...
    if (_isRefreshing) {
      _showTemporaryMessage(
        '버스 정보를 불러오는 중입니다. 잠시 후 다시 말해 주세요.',
      );
      return;
    }

    final number = spokenNumber.replaceAll(
      RegExp(r'\s+'),
      '',
    );

    final matches = _allBusList.where(
      (bus) {
  ...
  Future<void> _openBusDetail(
    ...
    if (boardingStop == null) {
      _showTemporaryMessage(
        '승차 정류장 정보를 확인할 수 없습니다.',
      );
      return;
    }

    final result =
        await Navigator.push<BusDetailResult>(
      context,
      MaterialPageRoute(
        builder: (context) => BusDetailScreen(
          stop: boardingStop,
          bus: bus.copyWith(
            isFavorite: _isFavorite(bus),
          ),
        ),
      ),
    );
    ...
    final searchBusNumber =
        result.searchBusNumber;
  ...
  Future<void> _toggleFavorite(
    ...
    if (!_favoriteUpdatesInProgress.add(
      bus.routeKey,
    )) {
      return;
    }
    ...
    try {
      final isFavorite =
          await FavoriteBusService.toggleFavorite(
        bus.routeKey,
      ...
      if (mounted) {
        setState(() {
          _favoriteUpdatesInProgress.remove(
            bus.routeKey,
          );
        });
      } else {
  ...
  Widget _buildBusList() {
    ...
    final routeNumberCounts =
        <String, int>{};

    for (final bus
        in _displayedBusList) {
